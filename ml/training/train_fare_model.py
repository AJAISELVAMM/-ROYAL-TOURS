"""Train the Random Forest fair-fare classifier (offline).

Pipeline: one-hot encode categoricals, passthrough numerics, RandomForest.
Stratified train/test split, 5-fold CV, small grid search, full metric report.
Saves the fitted pipeline + metadata + report under ml/models/fare_model_v1/.
"""
...
MODEL_DIR = "ml/models/fare_model_v1"
MODEL_KEY = "fare_model_v1"

CATEGORICAL = ["vehicleType", "timeOfDay", "dayOfWeek", "trafficLevel", "weatherCondition"]
NUMERICAL = [
    "distanceKm", "durationMinutes", "baseFare", "perKmRate",
    "perMinuteRate", "surgeMultiplier", "quotedFare",
]
FEATURES = CATEGORICAL + NUMERICAL


def main():
    df = pd.read_csv(f"{MODEL_DIR}/fare_dataset.csv")
    X = df[FEATURES]
    y = df["fareStatus"]
    ...
    grid = GridSearchCV(pipeline, param_grid, cv=cv, scoring="f1_weighted", n_jobs=-1, verbose=0)
    ...
    metrics = {
        ...
        "overcharged_recall": round(report.get("OVERCHARGED", {}).get("recall", 0.0), 4),
        "per_class": {c: {
    ...
    # Feature importances (model-level, not causal).
    ...
    importance = sorted(
        zip(all_names, importances.tolist()), key=lambda x: -x[1]
    )[:10]
    ...
    save_json(f"{MODEL_DIR}/fare_model_report.json", report_json)
    ...
    print(json.dumps({
        ...
        "overcharged_recall": metrics["overcharged_recall"],
        "confusion_matrix": cm,
    }, indent=2))
